##Script: IBE-eQTL_GxM_INTACT_overlap.R
##Author: Shreya Nirmalan
##Last Modified: 5-6-24
##Purpose: to overlap GxM QTLs with eGenes thar are associated with IBD, CD and UC risk based on integration of GTEx colon eQTL data with GWAS and TWAS
...
write.csv(intact_sig, "IBD_eQTL_significant_INTACT_genes.csv")

#import abundance GxE results to make overlap
all_results_abundance <- read.table("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/GxM_abundance/GxE_results/GxE_abundance_19PCs_all_results.txt", sep="\t", stringsAsFactors=F, header=T, comment="")
FDR <- 0.1
...
#remove redundtant abundance GxM

res_sig <- res_sig %>% filter(!microbe %in% redundant_microbes)

res_sig <- res_sig %>% filter(!microbe_gene %in% c("Bacteria.Proteobacteria.Gammaproteobacteria.Pasteurellales.Pasteurellaceae_CCDC82"))
...
write.csv(intact_gxm_abundance, "IBD_eQTL_intact_gxm_abundance.csv")
...
write.csv(intact_sig_gxm_abundance, "IBD_eQTL_intact_sig_gxm_abundance_pval_overlap.csv")


##P/A GxM
res <- read.table("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/GxM_presence_absence/GxE_results/GxE_presence_absence_all_results_19PCs.txt", sep="\t", stringsAsFactors=F, header=T, comment="")
...
write.csv(intact_gxm_dichotomous, "IBD_eQTL_intact_gxm_dichotomous.csv")
...
write.csv(intact_sig_gxm_abundance, "IBD_eQTL_intact_sig_gxm_presence_absence_pval_overlap.csv")
...
microbes <- read.table("/wsu/home/groups/piquelab/IBD_eQTL/covariates/abundance_model_all_microbes_1-per-var-list.txt", sep="\t", stringsAsFactors=F, comment="")
...
for(i in 1:length(microbe_list)) {
	...
	fname=paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/GxM_abundance/GxE_results/GxE_abundance_19PCs_", microbe, ".txt")
  results <- read.table(fname, sep="\t", stringsAsFactors=F, header=T, comment="")
	...
	write.table(res_intact, file=paste0("INTACT_sig_genes_GxE_abundance_19PCs_", microbe, ".txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
write.table(res_summary, file=paste0("INTACT_sig_genes_GxE_abundance_19PCs_summary.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
for(t in 1:length(traits)) {
	trait <- traits[t]
	results <- read.table(paste0("INTACT_sig_genes_GxE_abundance_19PCs_", trait, ".txt"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
	...
	for(i in 1:nrow(res_sig)) {
		...
		png(paste0("INTACT_Abundance_GxM_19PCs_Interaction_eQTL_Scatterplot_", trait,"_",gene_symbol, ".png"))
...
for(t in 1:length(traits)) {
	...
	for(i in 1:nrow(res_sig)) {
		...
		df$genotype <- factor(df$genotype, levels=(c(paste0(gt_data$ref, "/", gt_data$ref), paste0(gt_data$ref, "/", gt_data$alt),paste0(gt_data$alt, "/", gt_data$alt))))
		png(paste0("INTACT_Abundance_GxM_19PCs_Interaction_eQTL_tertile_boxplot_", trait, "_",gene_symbol,".png"))
...
microbes <- read.table("/wsu/home/groups/piquelab/IBD_eQTL/FastQTL/Rectum_protein_coding_residuals/GxE_dichotomous_19PCs/1-per-var-list.txt", sep="\t", stringsAsFactors=F, comment="")
...
for(i in 1:length(microbe_list)) {
	...
	fname=paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/GxM_presence_absence/GxE_results/GxE_presence_absence_19PCs_", microbe, ".txt")
	results <- read.table(fname, sep="\t", stringsAsFactors=F, header=T, comment="")
	...
	write.table(res_intact, file=paste0("INTACT_sig_genes_GxE_presence_absence_19PCs_", microbe, ".txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
write.table(res_summary, file=paste0("INTACT_sig_genes_GxE_presence_absence_19PCs_summary.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
for(t in 1:length(traits)) {
	trait <- traits[t]
	results <- read.table(paste0("INTACT_sig_genes_GxE_presence_absence_19PCs_", trait, ".txt"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
	...
	for(i in 1:nrow(res_sig)) {
		...
		df$genotype <- factor(df$genotype, levels=(c(paste0(gt_data$ref, "/", gt_data$ref), paste0(gt_data$ref, "/", gt_data$alt),paste0(gt_data$alt, "/", gt_data$alt))))
		png(paste0("INTACT_Interaction_eQTL_plots_", trait,"_", gene_symbol, "_GxE_presence_absence_19PCs.png"))
...
#abundance GxM
signif <- read.table("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_GxE_abundance_19PCs_summary.txt", sep="\t", stringsAsFactors=FALSE)
...
results <- read.table(paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_GxE_abundance_19PCs_", traits[1], ".txt"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
for(t in 1:length(traits)) {
	trait <- traits[t]
	results <- read.table(paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_GxE_abundance_19PCs_", trait, ".txt"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
signif <- read.table("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_GxE_presence_absence_19PCs_summary.txt", sep="\t", stringsAsFactors=FALSE)
colnames(signif) <- c("microbe", "sigGxM")
...
results <- read.table(paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_GxE_presence_absence_19PCs_", traits[1], ".txt"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
results$microbe <- traits[1]
...
for(t in 1:length(traits)) {
	trait <- traits[t]
	results <- read.table(paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_GxE_presence_absence_19PCs_", trait, ".txt"), sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
INTACT_sig <- read.csv("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/IBD_eQTL_significant_INTACT_genes.csv")
...
write.table(INTACT_sig_GxM, file=paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_with_GxM_full_results.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
write.table(INTACT_sig_GxM_short, file=paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_with_GxM_condensed_results.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)

write.csv(INTACT_sig_GxM_short, file=paste0("/rs/rs_grp_ibdeqtl/FastQTL/rectum_protein_coding_residuals/qnorm_residuals_analysis/INTACT_analysis/GxM_INTACT_overlap/INTACT_sig_genes_with_GxM_condensed_results.csv"))
