...
fn <- "IBD_Rectum_nominals_all_chunks.txt.gz"
...
opfn <- gzfile(paste(outdir, "IBD_Rectum.eQTL.txt.gz", sep=""))
...
opfn <- "geneList.txt"
write.table(geneList, file=opfn, quote=F, row.names=F, col.names=F)

##
snpList <- unique(res$V2)
opfn <- "snpList.txt"
write.table(snpList, file=opfn, quote=F, row.names=F, col.names=F)
